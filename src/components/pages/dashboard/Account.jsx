...
const Account = (props) => {
	...
	const uploadImage = async (image, id) => {
		...
		await Axios.put(
			`${process.env.REACT_APP_API_URL}/users/upload-logo/${id}`,
			data,
			config
		)
			.then(async (resp) => {
			...
			<div className="ui-text-center rt--bx">
				<div className="dp--bx">
					...
					<span className={`edit ${imgSource ? "round" : ""}`}>
						<Link onClick={(e) => openDialog(e)} to="">
							<i className="fe fe-edit-3"></i>
						</Link>
					</span>
					{userContext.user.logo && (
						<>
							{imgSource && <img src={imgSource} alt="avatar" />}
							{!imgSource && (
								<img
									src={
										userContext.user.logo === "no-logo.jpg"
											? "../../../images/assets/avatar-bag.svg"
											: userContext.user.logo
									}
									alt="avatar"
								/>
				...
				<h3 className="mrgb0 onmineshaft fs-20 font-weight-bold">
					...
					{!userContext.loading && (
						<>
							{" "}
							{userContext.user
								? userContext.user.resturantName
								: "No Name"}{" "}
						</>
					)}
				</h3>
				<p class="mrgb0 ui-text-center text-muted fs-14 mt">
					...
					{!userContext.loading &&
						...
						userContext.user.username && (
							<>
								<a
									href={`https://checkaam.com/${userContext.user.username}`}
									target="_blank"
									className="brand-orange fs-13 font-weight-medium"
								>
									www.checkaam.com/{userContext.user.username}
								</a>
							</>
						)}
					{!userContext.loading &&
					...
					<TabPanel tabId="three">
						<div className="dash-empty">
							<div className="pdl3 pdr3 ui-text-center">
								<img
									src="../../../images/assets/icon@ad.svg"
									width="110px"
									alt="ad"
								/>
								<p className="text-muted fs-14 ui-text-center mrgb1 mrgt2">
									Running Ads on Checkaam is currently unavailable. Please check
									back later
								</p>
							</div>
						</div>
							...
							<div className="dash-empty">
								<div className="pdl3 pdr3 ui-text-center">
									...
									<p class="mrgb0 ui-text-center">
										<span className="text-muted fs-14 onmineshaft">
											Status:{" "}
										</span>
										<span className="text-muted fs-14 onapple font-weight-bold">
											Active{" "}
										</span>
									</p>
								</div>
