...
const RestDetails = (props) => {
	...
	useEffect(async () => {
		...
		foodItemContext.getRestFoodItems(
			storage.checkHex(restId)
				? restId
				: storage.checkRestaurantID()
				? storage.getRestaurantID()
				: restId
		);
		addressContext.getRestAddresses(
			storage.checkHex(restId)
				? restId
				: storage.checkRestaurantID()
				? storage.getRestaurantID()
	...
	const checkErr = () => {
		setAData({
			...
			show: true,
			type: "danger",
			message: `You cannot order food from different locations.`,
		});
		setTimeout(() => {
	...
	const selectFood = async (e, data) => {
		...
		if (data.action === "add") {
			setSelected({
				...
				total: selected.total + data.price,
				items: selected.items.concat(data.item),
			});
		}
	...
	const getLoc = (data) => {
		...
		if (!locationContext.loading && data.location) {
			const loc = locationContext.locations.find(
				(l) => l._id === data.location
			);
			name = loc.name;
		}
		return name;
	...
	return (
		<>
						...
						<div className="">
							<div className="row">
								<div data-aos={"fade-right"} className="col-md-7 mx-auto">
									<div className="">
										<SearchBox
											foodOptions={foodContext.allFood}
			...
			{!userContext.loading &&
				storage.checkObject(userContext.restaurant) <= 0 && (
					<>
						<div className="load--bx food-empty mrgt5">
							...
							<div className="row ui-text-center pdr4 pdl4 mrgt1">
								<p className="onmineshaft fs-14 mrgt2 font-metromedium">
									Restaurant not found! Are you sure the username is correct?
								</p>
							</div>
						</div>
					</>
					...
					<div className="container">
						<div className="restaurant details">
							<div className="details">
								<p
									className="title font-metromedium fs-15 mt-2"
									style={{ color: colors.primary.green }}
								>
									{!userContext.restaurant.resturantName && (
								...
								{!addressContext.loading &&
									addressContext.restAddresses.length > 0 && (
										<p className="address font-metromedium">
											<>
												{getLoc(addressContext.restAddresses[0])}
												<span
													className={`${
														userContext.restaurant.isOpen
															? "onapple "
															: "onaliz"
													} fs-11 font-metromedium`}
													style={{ position: "relative", top: "1px" }}
												>
													&nbsp;&nbsp;
													{userContext.restaurant.isOpen ? "Open" : "Closed"}{" "}
													&nbsp;
												</span>
											</>
										</p>
							...
							<div className="ml-auto pdr1 ui-group-button mt-2">
								<Link onClick={(e) => openAddress(e)} to="" className="blink">
									{blink ? (
										...
										<span className="blink-circle init-blink"></span>
									)}
									<span className="fe fe-map-pin brand-orange fs-20"> </span>
								</Link>

								<a
									href={`tel: ${userContext.restaurant.phoneNumber}`}
									style={{ position: "relative", right: "-4px" }}
								>
									<span className="fe fe-phone onapple fs-20"></span>
								</a>
						...
						<div id="pubfood" className="food-items mrgt1 mrgb">
							<div className="d-flex align-items-center mrgb1">
								<h3
									className="title fs-15 font-metrolight mrgb0"
									style={{ color: colors.primary.green }}
								>
									Food Items
								</h3>
										...
										{empty ? (
											<>
												<div className="load--bx food-empty">
													...
													/>
													<div className="row ui-text-center pdr4 pdl4 mrgt1">
														<p className="onsilver">
															{userContext.restaurant.resturantName} does not
															have any food items
														</p>
													</div>
												</div>
							...
							{!foodItemContext.loading &&
								...
								foodItemContext.restFoodItems.length <= 0 && (
									<>
										<div className="load--bx food-empty">
											<img
												src="../../../images/icons/dplate-c.svg"
												className="empty mrgt2"
												alt="empty plate"
											/>
											<div className="row ui-text-center pdr4 pdl4 mrgt1 font-metromedium">
												<p className="onmineshaft">
													{userContext.restaurant.resturantName} does not have
													any food items at {locName}{" "}
												</p>
											</div>
										</div>
									</>
									...
									foodItemContext.restFoodItems.map((f, i) => (
										<>
											<FoodItem
												...
												loading={locationContext.loading}
												bg={colors.accents[i]}
												index={i}
												locations={locationContext.locations}
												foodItem={f}
			...
			>
				<div className="container">
					<div className="d-flex align-items-center">
						<h3
							className="title fs-16 mrgb1 font-metromedium"
							style={{ color: colors.primary.green }}
						>
							Addresses
						</h3>
						<div className="ml-auto">
							...
							>
								<span className="fe fe-x fs-18 brand-orange"></span>
							</Link>
						</div>
					</div>
								...
								<div className="add-item">
									<div className="d-flex align-items-center">
										...
										<p className="mrgb0 onmineshaft fs-14 font-weight-medium font-metrolight">
											...
											{!locationContext.loading ? getLoc(ad) : ""}
										</p>
									</div>
									<p className="mrgb0 text-muted font-metromedium">
										{ad.address}{" "}
									</p>
								</div>
			...
			<BottomBar
				...
				restId={userContext.restaurant._id}
			/>
		</>
	);
};
